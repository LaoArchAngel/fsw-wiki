...
#### Follow-Up: Who can XSRF be mitigated?

#### A:  
* per-request, unique Xsrf token passed with each request (usually one or all parts are generated by the server on previous server response)
* re-authenticate the user - made stronger by a second factor
* response body signing (maybe using HMAC and a secret key)
* verify referer and/or origin header
